...
#define NR_SECTS(p)	swab32(get_unaligned(&p->nr_sects))
#define START_SECT(p)	swab32(get_unaligned(&p->start_sect))
...
static void extended_partition(struct gendisk *hd, kdev_t dev)
{
	...
	while (1) {
	   ...
	   */
		...
		if (swab16(*(unsigned short *) (bh->b_data+510)) != 0xAA55)
			goto done;
...
static void bsd_disklabel_partition(struct gendisk *hd, kdev_t dev)
{
	...
	if (swab32(l->d_magic) != BSD_DISKMAGIC) {
		brelse(bh);
		return;
	}
	...
	while (p - &l->d_partitions[0] <= BSD_MAXPARTITIONS) {
		...
		if (p->p_fstype != BSD_FS_UNUSED) {
			add_partition(hd, current_minor, swab32(p->p_offset), swab32(p->p_size));
			current_minor++;
		}
		p++;
	}
...
check_table:
#endif
	if (swab16(*(unsigned short *)  (0x1fe + data)) != 0xAA55) {
		brelse(bh);
		return 0;
	}
	...
	if (!tested_for_xlate++) {	/* Do this only once per disk */
		...
		unsigned int sig = swab16(*(unsigned short *)(data + 2));
		...
		} else if (sig <= 0x1ae && swab16(*(unsigned short *)(data + sig)) == 0x55AA
	 ...
	 */
	if (swab16(*(unsigned short *) (data+0xfc)) == 0x55AA) {
		p = (struct partition *) (0x1be + data);
