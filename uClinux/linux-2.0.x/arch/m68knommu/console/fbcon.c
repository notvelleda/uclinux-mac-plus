 ...
 *
 * 6 bit wide monochrome font support by velleda
 * 
 * Based on:
 *
...
extern struct fb_info *mac_fb_init();
...
#define MAC_CURSOR_BLINK_RATE     (40)
...
static u_long fbcon_startup(u_long kmem_start, char **display_desc)
{
   ...
   *display_desc = fb_info->modename;
   ...
   // interrupt is handled in arch/m68knommu/platform/68000/config.c (for now)
#endif
...
fail:
   ...
   if ((p->dispsw == &dispsw_mono) && (p->fontwidth != 8) && (p->fontwidth != 6) && (p->fontwidth != 4) )
      panic("fbcon_setup: No support for fontwidth != 8/6/4 on monochrome targets");
...
static int fbcon_set_font(struct vc_data *conp, int w, int h, char *data)
{
	...
	if ((p->dispsw != &dispsw_mono) && (w != 8))
		...
		return( -ENXIO );
	if ((p->dispsw == &dispsw_mono) && (w != 8) && (w != 4) && (w != 6))
		return( -ENXIO );
                       ...
                       int height, int width)
{
   ...
   } else if (p->fontwidth == 6) {
      if ((sx == 0) && (width == ((p->next_line * 8) / 6))) {
         src = p->screen_base + sy * p->fontheight * p->next_line;
         dest = p->screen_base + dy * p->fontheight * p->next_line;
         mymemmove(dest, src, height * p->fontheight * p->next_line);
      } else {
         int x, y, ry;
         for (x = 0; x < width; x ++) {
            u_char *src = p->screen_base + sy * p->fontheight * p->next_line + ((sx + x) >> 2) * 3;
            u_char *dest = p->screen_base + dy * p->fontheight * p->next_line + ((dx + x) >> 2) * 3;

            int rs = (sx + x) & 0x3;
            int rd = (dx + x) & 0x3;

            for (y = 0; y < height; y ++) {
               for (ry = 0; ry < p->fontheight; ry ++, src += p->next_line, dest += p->next_line) {
                  u_char d;
                  ...
                  switch (rd) {
                     case 0:
                        *(dest + 0) = (*(dest + 0) & ~mask6[0]) | (d & mask6[0]);
                        break;
                     case 1:
                        *(dest + 0) = (*(dest + 0) & ~mask6[3]) | (d >> 6);
                        *(dest + 1) = (*(dest + 1) & ~mask6[4]) | (d << 2);
                        break;
                     case 2:
                        *(dest + 1) = (*(dest + 1) & ~mask6[7]) | (d >> 4);
                        *(dest + 2) = (*(dest + 2) & ~mask6[8]) | (d << 4);
                        break;
                     case 3:
                        *(dest + 2) = (*(dest + 2) & ~mask6[11]) | (d >> 2);
                        break;
                  }
               }
            }
                       ...
                       int height, int width)
{
   ...
   if (p->fontwidth == 8) {
      if (sx == 0 && width == p->next_line)
         if (attr_reverse(p, conp))
            memset(dest, 0xff, height * p->fontheight * width); // mymemset is broken!
         else
   ...
   } else if (p->fontwidth == 6) { /* not pretty but it works! */
      if (sx == 0 && width == p->next_line)
         if (attr_reverse(p, conp))
            memset(dest, 0xff, height * p->fontheight * width); // mymemset is broken!
         else
         ...
         dest = p->screen_base + sy * p->fontheight * p->next_line + (sx >> 2) * 3;
         dest2 = p->screen_base + sy * p->fontheight * p->next_line + ((sx + width - 1) >> 2) * 3;
         
         for (rows = height * p->fontheight; rows --; dest += p->next_line, dest2 += p->next_line) {
            int width2;
            u_char *dest3 = dest;
            ...
            if (width > 1) {
               width2 = dest2 - dest3;
               switch ((sx + width - 1) & 0x3) {
                  case 0:
                     *(dest2 + 0) = (*(dest2 + 0) & ~mask6[0]) | (d & mask6[0]);
                     break;
                  case 1:
                     *(dest2 + 0) = (*(dest2 + 0) & ~mask6[3]) | (d >> 6);
                     *(dest2 + 1) = (*(dest2 + 1) & ~mask6[4]) | (d << 2);
                     width2 ++;
                     break;
                  case 2:
                     *(dest2 + 1) = (*(dest2 + 1) & ~mask6[7]) | (d >> 4);
                     *(dest2 + 2) = (*(dest2 + 2) & ~mask6[8]) | (d << 4);
                     width2 += 2;
                     break;
                  case 3:
                     *(dest2 + 2) = (*(dest2 + 2) & ~mask6[11]) | (d >> 2);
                     width2 += 2;
                     break;
               }
            }
            ...
            if (width > 2)
               if (attr_reverse(p, conp))
                  memset(dest3, 0xff, width2);
               else
                  mymemclear(dest3, width2);
         }
      }
   }
}
                      ...
                      int x)
{
   ...
   if (p->fontwidth == 8) {
      ...
      dest = p->screen_base + y * p->fontheight * p->next_line + (x >> 2) * 3;
      ...
      for (rows = p->fontheight; rows --; dest += p->next_line) {
         ...
         *((u_long *) dest) = (*((u_long *) dest) & ~mask6[r]) | (d2 << (6 + r * 3));*/
      }
   }
}
                       ...
                       int count, int y, int x)
{
   ...
   if (p->fontwidth == 8) {
      ...
      dest0 = p->screen_base + y * p->fontheight * p->next_line + (x >> 2) * 3;
