...
static void inode_free(inode_build_t * inode)
{
	...
	entry = (entry_build_t *) inode->entries.node.next;
	...
	char *parent_path)
{
	...
	while (1) {
		...
		while (1) {
			...
			name = ent->d_name;
			child_ent = entry_alloc(parent_inode, name);
			if (!child_ent) {
				err = -1;
				break;
			}
			child_path = malloc(strlen(parent_path) + 1 + strlen(name) + 1);
...
//printf("REPARSE %s\n", child_path);
					...
					if (whole_disk_copy && destination_dir[0] == '/') {
						...
						if (p) {
							int old = *p, same; *p = 0;
							same = !strcmp(child_path, destination_dir);
							*p = old;
							if (same) {
								printf("Skipping %s\n", child_path);
								child_inode->flags = 0;
...
int copy_directory(char *source_dir, char *dest_dir)
{
	...
	list_init(&inodes);
	root_node = inode_alloc(NULL, source_dir);
	if (!root_node)
		return 1;
	...
	inode = (inode_build_t *) inodes.node.next;
