...
/* 
 * makes a human readable string, given a size and block size. setting display_unit to 1 will prevent the units from being appended?
 * literally just copied from busybox lmao
 */
    ...
    *cp = '\0';
    ...
    if (flags & LSF_LONG) {
        ...
        if (S_ISBLK(statbuf->st_mode) || S_ISCHR(statbuf->st_mode))
            ...
            sprintf(cp, "%8s ", make_human_readable_str((unsigned long)statbuf->st_size, 1, 0));
        else
...
usage:
    fprintf(stderr, "usage: %s [-aAFiltSrR1Uh] [name ...]\n", name);
    return EXIT_FAILURE;
}
